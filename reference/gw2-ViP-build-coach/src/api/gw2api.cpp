...
/* Single-call fetch using /v2/characters/{name}.
 * Parses WvW-specific specializations and skills, plus all equipment.
 * Falls back to the old multi-call path if this endpoint fails. */
static bool FetchCharacterAll(const std::string& api_key,
                              const std::string& character_name,
                              GW2::PlayerBuild&  out_build)
{
    std::string enc = UrlEncodeName(character_name);
    /* v=2019-02-21 ensures "stats" objects include an "id" field.
     * WvW specializations/skills are present in all post-2018 schema versions. */
    auto resp = Http::GetWithBearer(HOST,
        BuildPath("/v2/characters/" + enc + "?v=2019-02-21T00:00:00.000Z"), api_key);
    if (!resp.ok()) {
        char buf[256];
        snprintf(buf, sizeof(buf),
                 "GW2API: /v2/characters fetch failed for %s (HTTP %d: %s)",
                 character_name.c_str(), resp.status_code, resp.error.c_str());
        Log(LOGL_WARNING, buf);
        return false;
    }
    ...
    try {
        ...
        /* ── WvW Specializations / Traits ─────────────────────────────── */
        if (j.contains("specializations") &&
            j["specializations"].contains("wvw")) {
            auto& specs = j["specializations"]["wvw"];
            for (int i = 0; i < 3 && i < (int)specs.size(); i++) {
                auto& spec = specs[i];
                GW2::SpecLine line;
                line.spec_id = spec.value("id", 0u);
                if (spec.contains("traits")) {
                    auto& traits = spec["traits"];
                    for (int t = 0; t < 3 && t < (int)traits.size(); t++)
                        line.traits[t].trait_id =
                            traits[t].is_null() ? 0u : (uint32_t)traits[t];
                }
                out_build.traits.lines[i] = line;
                if (i == 2) out_build.elite_spec = EliteSpecFromInt(line.spec_id);
            }
        ...
        /* ── WvW Skills ───────────────────────────────────────────────── */
        if (j.contains("skills") && j["skills"].contains("wvw")) {
            auto& s = j["skills"]["wvw"];
            out_build.skills.heal  = s.value("heal",  0u);
            out_build.skills.elite = s.value("elite", 0u);
            if (s.contains("utilities")) {
                auto& utils = s["utilities"];
                for (int u = 0; u < 3 && u < (int)utils.size(); u++)
                    if (!utils[u].is_null())
        ...
        /* ── Equipment (gear + relic already included) ────────────────── */
        if (j.contains("equipment")) {
            out_build.gear.items.clear();
            out_build.gear.relic_id = 0;
            ParseEquipmentArray(j["equipment"], out_build, false);
        }

        /* Fallback: some API responses put the Relic inside a tab rather than
         * at the top level — scan equipment_tabs if we still have no relic. */
        if (!out_build.gear.relic_id &&
            j.contains("equipment_tabs") && j["equipment_tabs"].is_array()) {
        ...
        Log(LOGL_WARNING,
            (std::string("GW2API: /v2/characters parse error: ") + e.what()).c_str());
        return false;
    }
}
...
bool FetchFullPlayerBuild(const std::string& api_key,
                          ...
                          GW2::PlayerBuild&  out_build)
{
    /* Try the unified single-call endpoint first */
    if (FetchCharacterAll(api_key, character_name, out_build))
        return true;

    /* Fall back to the old multi-call path */
    Log(LOGL_WARNING, "GW2API: falling back to buildtabs+equipmenttabs");
