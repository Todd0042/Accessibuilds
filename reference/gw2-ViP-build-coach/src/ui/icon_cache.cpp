...
void DeployBundledIcons()
{
    if (s_icons_dir.empty()) return;
    for (size_t i = 0; i < BUNDLED_ICON_COUNT; i++) {
        std::string path = s_icons_dir + BUNDLED_ICONS[i].name + ".png";
        if (GetFileAttributesA(path.c_str()) != INVALID_FILE_ATTRIBUTES) continue;
        std::ofstream f(path, std::ios::binary);
        if (f.is_open())
            f.write(reinterpret_cast<const char*>(BUNDLED_ICONS[i].data),
                    static_cast<std::streamsize>(BUNDLED_ICONS[i].size));
    }
}

Texture_t* GetLocalTexture(const char* key)
{
    if (!APIDefs || !key || !key[0] || s_icons_dir.empty()) return nullptr;
    std::string path = s_icons_dir + key + ".png";
    if (GetFileAttributesA(path.c_str()) == INVALID_FILE_ATTRIBUTES) return nullptr;
    return APIDefs->Textures_GetOrCreateFromFile(key, path.c_str());
}
