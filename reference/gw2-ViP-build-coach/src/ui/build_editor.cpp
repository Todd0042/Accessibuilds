...
static void BuildToForm(const GW2::SCBuild& b)
{
    ...
    /* Relic: prefer relic_db name lookup over ItemLookup (relics aren't in that cache). */
    if (b.gear.relic_id) {
        const char* rn = FindRelicName(b.gear.relic_id);
        if (rn) strncpy(s_relic_name, rn, sizeof(s_relic_name)-1);
        else fmtC(s_relic_name, sizeof(s_relic_name), b.gear.relic_id);
    }
...
static GW2::SCBuild FormToBuild()
{
    ...
    /* Relic names are not in the ItemLookup cache, so try relic_db first. */
    if (!s_relic_name[0]) {
        b.gear.relic_id = 0;
    } else if (IsAllDigits(s_relic_name)) {
        b.gear.relic_id = (uint32_t)atoi(s_relic_name);
    } else {
        b.gear.relic_id = FindRelicID(s_relic_name);
        if (!b.gear.relic_id) b.gear.relic_id = ItemLookup::FindItemID(s_relic_name);
        if (!b.gear.relic_id && s_relic_id_override[0] && IsAllDigits(s_relic_id_override)) {
            b.gear.relic_id = (uint32_t)atoi(s_relic_id_override);
            if (b.gear.relic_id) ItemLookup::CacheItemName(s_relic_name, b.gear.relic_id);
        }
    }
