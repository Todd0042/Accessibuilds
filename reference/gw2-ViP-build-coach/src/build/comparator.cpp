...
static const char* WeaponTypeString(GW2::WeaponType wt)
{
    switch (wt) {
    ...
    case GW2::WeaponType::Speargun:   return "HarpoonGun";
                             ...
                             const GW2::GearBuild& reference)
{
    ...
    for (const auto& pi : player.items) {
        ...
        } else if (!ri.upgrade_id && !ri.upgrade_name.empty() && pi.upgrade_id) {
            /* Reference uses text name only — compare once the API name has resolved */
        ...
        /* Weapon type — only checked for weapon slots when reference specifies a type */
        if (ri.weapon_type != GW2::WeaponType::Unknown && pi.item_id) {
            using GS = GW2::GearSlot;
            if (pi.slot == GS::WeaponA1 || pi.slot == GS::WeaponA2 ||
                pi.slot == GS::WeaponB1 || pi.slot == GS::WeaponB2) {
                std::string api_type = GW2Names::GetItemType(pi.item_id);
                if (!api_type.empty() && api_type != "...") {
                    const char* ref_str = WeaponTypeString(ri.weapon_type);
                    size_t ref_len = strlen(ref_str);
                    bool type_ok = (api_type.size() == ref_len);
                    for (size_t k = 0; type_ok && k < ref_len; k++)
                        if (tolower((unsigned char)api_type[k]) != tolower((unsigned char)ref_str[k]))
                            type_ok = false;
                    if (!type_ok)
                        AddDiff(result, Severity::Error, "Gear",
                                "WeaponType_" + std::to_string((int)pi.slot),
                                api_type, ref_str,
                                "Wrong weapon type on " + SlotName(pi.slot));
                }
            }
        }
    }
