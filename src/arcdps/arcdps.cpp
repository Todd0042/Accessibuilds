...
void OnLocalCombatEvent(void* aEventArgs)
{
    ...
    /* Advance per-second histories — DPS, heal, and barrier */
    {
        ...
        while (fight_sec > s_history_sec) {
            float sec_dps = (float)s_sec_damage;
            s_dps_history.push_back(sec_dps);
            if (sec_dps > s_peak_dps) s_peak_dps = sec_dps;
            s_sec_damage = 0;
            ++s_history_sec;
        }
        while (fight_sec > s_hs_history_sec) {
            float sec_hps = (float)s_sec_heal;
            float sec_bps = (float)s_sec_barrier;
            s_heal_history.push_back(sec_hps);
            s_barrier_history.push_back(sec_bps);
            if (sec_hps > s_peak_hps) s_peak_hps = sec_hps;
            s_sec_heal    = 0;
            s_sec_barrier = 0;
            ++s_hs_history_sec;
        }
